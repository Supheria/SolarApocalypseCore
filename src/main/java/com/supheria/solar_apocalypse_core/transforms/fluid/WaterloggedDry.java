...
public class WaterloggedDry {
    ...
    public static final BlockTransform TRANSFORM = TransformRule.rulesOf(
            when(stageExact(SolarStage.STAGE_2).and(sky()).and(daytime()).and(IS_WATERLOGGED), DEWATERLOG),
            when(stageExact(SolarStage.STAGE_3).and(daytime()).and(aboveWaterEvaporateHeight()).and(IS_WATERLOGGED), DEWATERLOG),
