...
public class WaterEvaporate {
    ...
    public static final BlockTransform TRANSFORM = TransformRule.rulesOf(
            ...
            when(stageExact(SolarStage.STAGE_2).and(sky()).and(daytime()).and(aboveWaterEvaporateHeight()).and(randomDayRate()).and(stageRateScaled(0.55)),
                    spreadWaterRateLimited(BlockSpreadUtils.OFFSETS_17, 0.6)),
            // 阶段3：对白天高于安全高度的水进行快速蒸发，但仍保持17邻限额推进以控制卡顿
            when(stageExact(SolarStage.STAGE_3).and(daytime()).and(aboveWaterEvaporateHeight()).and(stageRateScaled(1.0)),
                    spreadWaterRateLimited(BlockSpreadUtils.OFFSETS_17, 1.0)),
            // 阶段4：晴天且超过蒸发高度，升级为更大范围的5×5×4层蒸发
            when(stageExact(SolarStage.STAGE_4).and(daytime()).and(aboveWaterEvaporateHeight()).and(stageRateScaled(0.9)),
